...
- Agent and tools: `agents/stratamind-reasoner/` contains supporting agent code; `src/stratamind/` contains the client-side Reasoner logic.
...
- Client logic for AI: `src/stratamind/Reasoner.ts` + `src/stratamind/config.ts` (environment-driven feature flags) + `src/stratamind/Reasoner.test.ts`.
...
- Lint: `npm run lint`.
- Tests: `npm run test` (Vitest). Key tests: `src/stratamind/Reasoner.test.ts` demonstrates mocking `fetch` using `vi`.
- Functions local dev:
...
- Client-side env vars follow Vite prefix `VITE_`. DO NOT put secrets in `VITE_*` (they are baked into builds).
  - `VITE_STRATAMIND_ENDPOINT`: override endpoint (defaults to `/.netlify/functions/reasoner`).
  - `VITE_STRATAMIND_ENABLED`: `true` or `false` toggles simulation vs. endpoint lookup.
  - `VITE_STRATAMIND_TIMEOUT`: timeout for backend call (ms).
...
- Server side secrets should be stored as environment variables in the hosting provider:
  - `STRATAMIND_API_KEY` or `OPENAI_API_KEY` for real AI responses.
...
- Client-server boundary: `src/stratamind/Reasoner.ts` will attempt a POST to `VITE_STRATAMIND_ENDPOINT` first, and then fall back to a local simulation. When testing features, prefer to run the local function or deploy the simple serverless function.
- Simulation: The repo intentionally provides safe, deterministic simulation responses to enable offline UI development (see `callReasoner` in `src/stratamind/Reasoner.ts`). Tests expect the simulation to match a few phrases.
...
- Firebase functions: `functions/index.js` — supports CORS and has a simulation fallback when `STRATAMIND_API_KEY` is missing.
...
- Add or modify Reasoner behaviors: update `src/stratamind/Reasoner.ts` and tests in `src/stratamind/Reasoner.test.ts`. Mocking fetch in tests is the canonical method for verifying behaviors.
...
- Client: `src/`, with primary components `src/App.tsx`, `src/components/ReasonerPanel.tsx` and `src/stratamind/*`.
- Serverless: `serverless/reasoner.js` and `functions/index.js`.
- Tests: `src/stratamind/Reasoner.test.ts`.
